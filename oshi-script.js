...
const defaultState = {
  ...
  location: "関東 / たまに遠征",
  locationLabel: "住んでる所",
  intro: "同担さんも他担さんもゆるく仲良くできたらうれしいです。返信はゆっくりめですが、推しの話をたくさんしたいです。",
  tags: "推し活, 同担歓迎, 現場参戦, グッズ収集",
  footerMessage: "推しの話をゆるく楽しくできたらうれしいです",
  layoutMode: "standard",
  sizePreset: "portrait",
  themePreset: "paper",
  frameStyle: "soft",
  ...
  surfaceColor: "#fffafc",
  surfaceColorEnd: "#fffdf7",
  labelTextColor: "#1b2326",
  textColor: "#1b2326",
  textStylePreset: "type01",
  oshiName: "星乃みお / 青担当",
  oshiGroup: "Lumiere / アイドル全般",
  oshiHistory: "2021年の配信で知って、そのまま沼落ちしました",
  oshiLove: "歌声と表情、ファンへの言葉選びがやさしいところが大好きです",
  checkSameWelcome: true,
  checkSameCareful: false,
  checkOtherWelcome: true,
  checkCallOk: false,
  checkDmOk: true,
  checkSlowReply: true,
  checkCasualTalk: false,
  checkOnsite: true,
  checkStreaming: true,
  checkGoods: true,
  checkNuiPhoto: false,
  ...
  checkCosplay: false,
  checkHandmade: false,
  checkSingleOshi: true,
  checkGroupOshi: false,
  checkDd: false,
  checkSpoilerOk: true,
  checkFujoshiOk: false,
  checkYume: false,
...
const checklistGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "checkDd", label: "DD" },
      { key: "checkSpoilerOk", label: "ネタバレOK" },
...
function loadSample() {
  state = constrainStateForPage({
    ...
    displayName: "すず / @suzu_live",
    displayNameVisible: "show",
    displayNameLabel: "HN",
    roleVisible: "show",
    roleLabel: "肩書き",
    role: "ライブも配信もどちらも追っています",
    genderVisible: "show",
    gender: "ひみつ",
    genderLabel: "性",
    bloodVisible: "show",
    ...
    location: "関西 / 遠征あり",
    locationLabel: "出没場所",
    intro: "現場も配信もどちらも好きです。同担さんも他担さんも大歓迎、ゆっくり仲良くなれたらうれしいです。",
    ...
    avatarShape: "rounded",
    accentColor: "#d95b8d",
    accentColorEnd: "#ff93b5",
    surfaceColor: "#fff9fc",
    surfaceColorEnd: "#fffdf6",
    textColor: "#1b2326",
    oshiName: "ユナ / ハオ",
    oshiGroup: "Starlight / 韓国アイドル",
    oshiHistory: "2022年のMVで知って、そのまま一気に沼落ちしました",
    oshiLove: "歌声と表情管理、ステージ上の空気づくりが大好きです",
    checkSameWelcome: true,
    checkSameCareful: false,
    checkOtherWelcome: true,
    checkCallOk: true,
    ...
    checkMultiFandom: true,
    checkNewFan: false,
    socialX: "https://x.com/suzu_live",
    socialXLabel: "X",
    socialInstagram: "https://www.instagram.com/suzu_live/",
    socialInstagramLabel: "Instagram",
    socialThreads: "",
    socialThreadsLabel: "通販",
    socialTikTok: "https://www.tiktok.com/@suzu_live",
    socialTikTokLabel: "TikTok",
    socialYouTube: "https://www.youtube.com/@suzu_live",
    socialBluesky: "",
    socialLine: "https://line.me/ti/p/@suzu_live",
    backgroundOverlayOpacity: 55,
    avatarDataUrl: "",
    backgroundImageDataUrl: "",
    backgroundImageFrontDataUrl: "",
...
function loadState() {
  try {
    ...
    const fallbackRaw = pageStorageKey ? localStorage.getItem("sns-oshi-self-intro-card-state") : null;
...
function ensureQrImage(url) {
  ...
  if (current && (current.status === "loading" || current.status === "loaded" || current.status === "error")) {
    return;
  }
...
function renderStandardCard() {
  ...
  const tagsTop = sectionBottom + canvas.width * 0.016;
  ...
  const gridTop = checklistBottom + canvas.width * (hasChecklistItems ? 0.01 : 0.012);
...
function drawQrTile(context, link, x, y, size, accent, labelText) {
  ...
  roundRect(context, x, y, size, size, Math.round(size * 0.18));
  context.fill();

  context.strokeStyle = hexToRgba(accent, 0.24);
  context.lineWidth = 1.6;
  roundRect(context, x, y, size, size, Math.round(size * 0.18));
  context.stroke();

  if (qrImage) {
    const inset = Math.max(Math.round(size * 0.08), 4);
    context.save();
    roundRect(context, x + inset, y + inset, size - inset * 2, size - inset * 2, Math.max(Math.round(size * 0.12), 6));
    context.clip();
  ...
  roundRect(context, x + 4, y + 4, size - 8, size - 8, Math.round(size * 0.14));
  context.fill();
  context.fillStyle = hexToRgba(accent, 0.8);
  context.textBaseline = "middle";
  context.font = getCanvasFont(clamp(Math.floor(size * 0.18), 9, 13), 800, "body");
  context.fillText("QR", x + size / 2, y + size / 2 - 4);
  context.font = getCanvasFont(clamp(Math.floor(size * 0.1), 7, 9), 700, "label");
...
function drawChecklistPanel(context, groups, x, y, maxWidth, accent, textColor, focusFieldName = "checkSameWelcome") {
  ...
  preparedGroups.forEach((group) => {
    ...
    cursorY = badges.length
      ? badges[badges.length - 1].y + badgeHeight + 12
      : headingY + 26;
  });

  const panelHeight = preparedGroups.length ? cursorY - y : 92;
  ...
  if (!preparedGroups.length) {
    context.fillStyle = hexToRgba(textColor, 0.72);
    context.font = getCanvasFont(16, 500, "body");
...
function drawOshiItemGrid(context, items, x, y, width, height, accent, textColor) {
  ...
  items.forEach((item, index) => {
    ...
    const titleFontBase = clamp(Math.floor(cardHeight * 0.13), 12, 17);
...
function collectOshiItems() {
  return [
    { label: "推し・担当", value: state.oshiName, fieldName: "oshiName" },
    { label: "推しグループ・界隈", value: state.oshiGroup, fieldName: "oshiGroup" },
    { label: "推し歴・きっかけ", value: state.oshiHistory, fieldName: "oshiHistory" },
    { label: "推しの好きなところ", value: state.oshiLove, fieldName: "oshiLove" }
  ];
}
...
function syncSocialOverlay() {
  ...
  socialOverlayItems.forEach((item) => {
    ...
    if (item.type === "external-link") {
      const link = document.createElement("a");
      link.className = "preview-hotspot preview-hotspot-link";
      link.href = item.url;
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.title = `${item.label} を開く`;
      link.setAttribute("aria-label", `${item.label} を開く`);
      link.style.left = `${item.x * scaleX}px`;
      link.style.top = `${item.y * scaleY}px`;
      link.style.width = `${item.width * scaleX}px`;
      link.style.height = `${item.height * scaleY}px`;
      link.style.borderRadius = `${item.radius * Math.min(scaleX, scaleY)}px`;
