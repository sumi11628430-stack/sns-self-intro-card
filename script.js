...
const STORAGE_KEY = pageStorageKey || "sns-self-intro-card-state";
...
const defaultState = {
  ...
  avatarRatio: "square",
  accentColor: "#ec7b46",
  accentColorEnd: "#f2a46f",
  surfaceColor: "#fff7ef",
  surfaceColorEnd: "#fffdf9",
  labelTextColor: "#1b2326",
  textColor: "#1b2326",
  textStylePreset: "type02",
  item1Label: "好き",
  item1Value: "文字組み / 余白 / 喫茶店",
  item2Label: "趣味",
  item2Value: "散歩 / 写真 / 雑貨集め",
  item3Label: "作業時間",
...
function loadSample() {
  state = constrainStateForPage({
    ...
    avatarShape: "rounded",
    accentColor: "#e47b4d",
    accentColorEnd: "#f0b07a",
    surfaceColor: "#fff8f1",
    surfaceColorEnd: "#fffdf8",
    textColor: "#1b2326",
    item1Label: "好き",
    item1Value: "紙もの / 喫茶店 / やわらかい色",
    item2Label: "趣味",
    item2Value: "散歩 / 読書 / 写真",
    item3Label: "作業時間",
    item3Value: "朝から昼にかけて元気です",
    item4Label: "ひとこと",
    item4Value: "気軽に話しかけてもらえるとうれしいです",
    socialX: "https://x.com/suzu_daily",
    socialXLabel: "X",
    socialInstagram: "https://www.instagram.com/suzu_daily/",
    socialInstagramLabel: "Instagram",
    socialThreads: "https://www.youtube.com/@suzu_daily",
    socialThreadsLabel: "YouTube",
    socialTikTok: "https://line.me/ti/p/@suzu_daily",
...
function loadState() {
  try {
    ...
    const fallbackRaw = pageStorageKey ? localStorage.getItem("sns-self-intro-card-state") : null;
...
function renderStandardCard() {
  ...
  if (metaItems.length) {
    ...
    const qrColumnWidth = socialLinks.length ? clamp(inner.w * 0.52, 480, 560) : 0;
    ...
    const qrColumnWidth = clamp(inner.w * 0.52, 480, 560);
    const qrLayout = drawStandardSocialQrGrid(
      ctx,
      socialLinks,
...
function collectOshiItems() {
  return [
    { label: state.item1Label || "項目1", value: state.item1Value, fieldName: "item1Value" },
    { label: state.item2Label || "項目2", value: state.item2Value, fieldName: "item2Value" },
    { label: state.item3Label || "項目3", value: state.item3Value, fieldName: "item3Value" },
    { label: state.item4Label || "項目4", value: state.item4Value, fieldName: "item4Value" }
  ];
}
